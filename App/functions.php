...
function a($station,$date){//per la previsione
 ...
 while ($index<$length){
    $type=$array_types[$index];
    $query="SELECT measurements.measure,measurements.date,sensortypes.type,sensortypes.pk_sensortype FROM devices join sensors on sensors.fk_device=devices.pk_device join measurements on measurements.fk_sensor=sensors.pk_sensor join sensortypes on sensortypes.pk_sensortype=sensors.fk_sensortype WHERE devices.name='$station' AND measurements.date BETWEEN '$date'-1209600000  AND '$date' AND sensortypes.type='$type' ORDER BY measurements.date ASC";
    $result=mysql_query($query);
	if (mysql_num_rows($result)>0){
      array_push($array_resultsets, $result);
...
function findMeasurementsMultipleResultSets($station,$date){//per la previsione
 ...
 while ($index<$length){
    $type=$array_types[$index];
    $query="SELECT measurements.measure,measurements.date,sensortypes.type,sensortypes.pk_sensortype FROM devices join sensors on sensors.fk_device=devices.pk_device join measurements on measurements.fk_sensor=sensors.pk_sensor join sensortypes on sensortypes.pk_sensortype=sensors.fk_sensortype WHERE devices.name='$station' AND measurements.date>='$date'-1209600000  AND sensortypes.type='$type' ORDER BY measurements.date ASC";
    $result=mysql_query($query);
	if (mysql_num_rows($result)>0){
      array_push($array_resultsets, $result);
...
function findStationsSpecificDate($city,$date){
 ...
 $stations_list=mysql_query("SELECT DISTINCT devices.name,measurements.date FROM devices JOIN cities ON devices.fk_city = cities.pk_city JOIN sensors ON devices.pk_device=sensors.fk_device JOIN sensortypes ON sensors.fk_sensortype=sensortypes.pk_sensortype JOIN measurements ON sensors.pk_sensor=measurements.fk_sensor WHERE cities.name='$city' AND measurements.date BETWEEN '$date' -86400000 AND '$date'");
...
function checkStation($station,$today){//with at least one measure
  ...
  $check=mysql_query("SELECT DISTINCT devices.name FROM devices JOIN cities ON devices.fk_city = cities.pk_city JOIN sensors ON devices.pk_device=sensors.fk_device JOIN sensortypes ON sensors.fk_sensortype=sensortypes.pk_sensortype JOIN measurements ON sensors.pk_sensor=measurements.fk_sensor WHERE devices.name='$station' AND measurements.date>'$today'-604800000 GROUP BY  devices.name HAVING COUNT(measurements.measure)>1");
...
function findDetails($station,$date){
  $query="SELECT DISTINCT sensortypes.type,measurements.measure FROM  devices JOIN sensors ON devices.pk_device=sensors.fk_device RIGHT OUTER JOIN measurements ON sensors.pk_sensor=measurements.fk_sensor JOIN  sensortypes ON  sensors.fk_sensortype=sensortypes.pk_sensortype WHERE devices.name='$station' AND measurements.date BETWEEN '$date' -86400000 AND '$date'";
...
function load_active_markers(){//with at least one measure
    ...
    $tab_devices=mysql_query("SELECT  devices.* FROM devices  JOIN sensors ON devices.pk_device=sensors.fk_device JOIN sensortypes ON sensors.fk_sensortype=sensortypes.pk_sensortype JOIN measurements ON sensors.pk_sensor=measurements.fk_sensor WHERE measurements.date>'$today'-604800000 GROUP BY  devices.name HAVING COUNT(measurements.measure)>1");
    $array_devices = array();
	while($row = mysql_fetch_assoc($tab_devices))
	{
		$row["name"] = utf8_encode($row["name"]);
...
function extract_measurements($deviceid){
	$array_sensors = array();
	$query1 = "SELECT DISTINCT sensors.fk_sensortype FROM sensors JOIN devices ON devices.pk_device = sensors.fk_device WHERE devices.pk_device=$deviceid";
...
//	$query2 ="SELECT measurements.measure, sensors.fk_sensortype, measurements.date FROM measurements JOIN sensors ON measurements.fk_sensor=sensors.pk_sensor JOIN devices ON sensors.fk_device=devices.pk_device WHERE devices.pk_device=$deviceid AND measurements.date  > ((SELECT MAX(date) FROM `measurements` JOIN sensors ON measurements.fk_sensor=sensors.pk_sensor JOIN devices ON sensors.fk_device=devices.pk_device WHERE devices.pk_device=$deviceid) - 86400000) ORDER BY measurements.date ASC, sensors.fk_sensortype ASC";
	$query2 ="SELECT measure, fk_sensortype, date FROM measureview_minimal WHERE fk_device=$deviceid AND date > ((SELECT MAX(date) FROM `measurements` JOIN sensors ON measurements.fk_sensor=sensors.pk_sensor JOIN devices ON sensors.fk_device=devices.pk_device WHERE devices.pk_device=$deviceid) - 86400000) ORDER BY date ASC, fk_sensortype ASC";
...
$cache_file_predicted= "/moniqa/tmp/get_color_predicted_cache.json";
$cache_expires = 3600;
...
function write_file_iqa(){
        ...
        $queryc ="SELECT devices.name, T.fk_device, measure, fk_sensortype, maxdate AS date
				FROM measureview_minimal JOIN (SELECT fk_device, MAX( DATE ) AS maxdate
																			FROM measureview_minimal GROUP BY fk_device ) T ON
																			T.fk_device = measureview_minimal.fk_device JOIN devices ON
																			T.fk_device = devices.pk_device 
																			WHERE date > maxdate - 86400000
																			GROUP BY fk_device, fk_sensortype, measure
																			ORDER BY T.fk_device ASC, fk_sensortype ASC, date DESC;";
																			//86400000 msec sono 1 giorno
		...
		//echo getcwd();
		fwrite($myfile, json_encode($array_colors));
		fclose($myfile);
